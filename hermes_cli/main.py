...
def cmd_whatsapp(args):
    ...
    if not (bridge_dir / "node_modules").exists():
        ...
        if result.returncode != 0:
            print(f"  [ERR] npm install failed: {result.stderr}")
            return
...
def _model_flow_copilot(config, current_model=""):
    ...
    if not api_key:
        ...
        print("    [ERR] Classic PAT (ghp_*)          NOT supported by Copilot API")
...
def _model_flow_bedrock(config, current_model=""):
    ...
    try:
        ...
        print("  [ERR] boto3 is not installed. Install it with:")
...
def _update_via_zip(args):
    ...
    try:
        ...
        print(f"[ERR] ZIP update failed: {e}")
        sys.exit(1)
...
def _restore_stashed_changes(
    ...
    if restore.returncode != 0 or has_conflicts:
        print("[ERR] Update pulled new code, but restoring local changes hit conflicts.")
...
def _sync_with_upstream_if_needed(git_cmd: list[str], cwd: Path) -> None:
    ...
    if not has_upstream:
        ...
        if response in ("", "y", "yes"):
            ...
            if _add_upstream_remote(git_cmd, cwd):
                ...
                print("  [ERR] Failed to add upstream remote. Skipping upstream sync.")
                return
        else:
            print(
    ...
    try:
        ...
        print("  [ERR] Failed to fetch upstream. Skipping upstream sync.")
        return
    ...
    if origin_ahead < 0 or upstream_ahead < 0:
        print("  [ERR] Could not compare branches. Skipping upstream sync.")
        return
    ...
    try:
        ...
        print(
            "  [ERR] Failed to pull from upstream. You may need to resolve conflicts manually."
        )
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    if not git_dir.exists():
        if sys.platform == "win32":
            ...
            print("[ERR] Not a git repository. Please reinstall:")
            print(
    ...
    try:
        ...
        if fetch_result.returncode != 0:
            ...
            if "Could not resolve host" in stderr or "unable to access" in stderr:
                print("[ERR] Network error - cannot reach the remote repository.")
                print(f"  {stderr.splitlines()[0]}" if stderr else "")
            elif (
                "Authentication failed" in stderr or "could not read Username" in stderr
            ):
                print(
                    "[ERR] Authentication failed — check your git credentials or SSH key."
                )

            else:
                print(f"[ERR] Failed to fetch updates from origin.")
                if stderr:
        ...
        try:
            ...
            if pull_result.returncode != 0:
                ...
                if reset_result.returncode != 0:
                    print(f"[ERR] Failed to reset to origin/{branch}.")
                                ...
                                if restart.returncode == 0:
                                    ...
                                    if verify.stdout.strip() == "active":
                                        ...
                                        if verify2.stdout.strip() == "active":
                                            ...
                                            print(
                                                f"  [ERR] {svc_name} failed to stay running after restart.\n"
