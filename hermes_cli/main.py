...
def cmd_whatsapp(args):
    ...
    if not current_mode:
        ...
        mode_label = (
            "separate bot number"
            if wa_mode == "bot"
            else "personal number (self-chat)"
        )
        print(f"\n✓ Mode: {mode_label}")
    ...
    if current_users:
        ...
        if phone:
            ...
            print(
                "  ⚠ No allowlist — the agent will respond to ALL incoming messages"
            )
...
def select_provider_and_model(args=None):
    ...
    _has_saved_custom_list = isinstance(
        config.get("custom_providers"), list
    ) and bool(config.get("custom_providers"))
...
def _model_flow_openrouter(config, current_model=""):
    ...
    if not api_key:
        ...
        try:
            ...
            key = getpass.getpass(
                "OpenRouter API key (or Enter to cancel): "
            ).strip()
        except (KeyboardInterrupt, EOFError):
            print()
            return
...
def _model_flow_custom(config):
    ...
    try:
        if len(detected_models) == 1:
            ...
            if confirm in ("", "y", "yes"):
                ...
                model_name = input(
                    "Model name (e.g. gpt-4, llama-3-70b): "
                ).strip()
        ...
        display_name = (
            input(f"Display name [{default_name}]: ").strip() or default_name
        )
    except (KeyboardInterrupt, EOFError):
        print("\nCancelled.")
        return
    ...
    if model_name:
        ...
        print(
            "Endpoint saved. Use `/model` in chat or `hermes model` to set a model."
        )
...
def _model_flow_named_custom(config, provider_info):
    ...
    if models:
        ...
        try:
            model_name = (
                input(f"Model name [{saved_model}]: ").strip() or saved_model
            )
        except (KeyboardInterrupt, EOFError):
            print("\nCancelled.")
            return
        ...
        print(
            "Could not fetch models from endpoint. Enter model name manually."
        )
        try:
    ...
    if provider_key:
        ...
        if isinstance(providers_cfg, dict):
            ...
            if isinstance(provider_entry, dict):
                ...
                if (
                    key_env
                    and not str(provider_entry.get("key_env", "") or "").strip()
                ):
...
def _prompt_reasoning_effort_selection(efforts, current_effort=""):
    ...
    while True:
        try:
            choice = input(
                f"Choice [1-{n + 2}] (default: keep current): "
            ).strip()
            if not choice:
                return None
            idx = int(choice)
...
def _model_flow_api_key_provider(config, provider_id, current_model=""):
    ...
    current_base = ""
    if base_url_env:
        current_base = get_env_value(base_url_env) or os.getenv(
            base_url_env, ""
        )
...
def _gateway_prompt(
    prompt_text: str, default: str = "", timeout: float = 300.0
) -> str:
...
def _count_commits_between(
    git_cmd: list[str], cwd: Path, base: str, head: str
) -> int:
    ...
    if not has_upstream:
        ...
        if response in ("", "y", "yes"):
            ...
            if _add_upstream_remote(git_cmd, cwd):
                ...
                print(
                    "  ✗ Failed to add upstream remote. Skipping upstream sync."
                )
                return
        else:
            print(
...
def cmd_update(args):
    ...
    try:
        ...
        needs_migration = (
            missing_env or missing_config or current_ver < latest_ver
        )
        ...
        if needs_migration:
            ...
            if missing_config:
                print(
                    f"  ℹ️  {len(missing_config)} new config option(s) available"
                )
            ...
            if gateway_mode:
                response = (
                    _gateway_prompt(
                        "Would you like to configure new options now? [Y/n]",
                        "n",
                    )
                    .strip()
                    .lower()
                ...
                print(
                    "  ℹ Non-interactive session — skipping config migration prompt."
                )
                print(
            ...
            if response in ("", "y", "yes"):
                ...
                results = migrate_config(
                    interactive=not gateway_mode, quiet=False
                )
                ...
                if gateway_mode and missing_env:
                    print(
                        "  ℹ API keys require manual entry: hermes config migrate"
                    )
            else:
                print()
                print(
                    "Skipped. Run 'hermes config migrate' later to configure."
                )
        else:
        ...
        try:
            ...
            if restarted_services or killed_pids:
                ...
                if killed_pids:
                    print(
                        f"  → Stopped {len(killed_pids)} manual gateway process(es)"
                    )
    ...
    if action == "list":
        ...
        try:
            ...
            if clone or clone_all:
                ...
                print(
                    f"    or it will inherit keys from your shell environment."
                )
                print(
...
""",
    ...
    gateway_status.add_argument(
        "--deep", action="store_true", help="Deep status check"
    )
    gateway_status.add_argument(
        "--system",
        action="store_true",
    ...
    auth_add.add_argument(
        "--ca-bundle", help="Custom CA bundle for OAuth login"
    )
    auth_list = auth_subparsers.add_parser(
        "list", help="List pooled credentials"
    )
    auth_list.add_argument(
        "provider", nargs="?", help="Optional provider filter"
    )
    ...
    auth_reset = auth_subparsers.add_parser(
        "reset",
        help="Clear exhaustion status for all credentials for a provider",
    )
    ...
    wh_sub.add_argument(
        "name", help="Route name (used in URL: /webhooks/<name>)"
    )
    wh_sub.add_argument(
        "--prompt",
        default="",
        help="Prompt template with {dot.notation} payload refs",
    )
    wh_sub.add_argument(
        "--events", default="", help="Comma-separated event types to accept"
    )
    wh_sub.add_argument(
        "--description", default="", help="What this subscription does"
    )
    wh_sub.add_argument(
    ...
    wh_test.add_argument(
        "--payload",
        default="",
        help="JSON payload to send (default: test payload)",
    )
...
""",
    ...
    config_set.add_argument(
        "key",
        nargs="?",
        help="Configuration key (e.g., model, terminal.backend)",
    )
    ...
    skills_install.add_argument(
        "--force",
        action="store_true",
        help="Install despite blocked scan verdict",
    )
    skills_install.add_argument(
        "--yes",
        "-y",
    ...
    snap_import.add_argument(
        "--force",
        action="store_true",
        help="Force install despite caution verdict",
    )
    ...
    def cmd_skills(args):
        ...
        if getattr(args, "skills_action", None) == "config":
            ...
            from hermes_cli.skills_config import (
                skills_command as skills_config_command,
            )
    ...
    mcp_add_p.add_argument(
        "--auth", choices=["oauth", "header"], help="Auth method"
    )
    ...
    sessions_export.add_argument(
        "--session-id", help="Export a specific session"
    )
    ...
    sessions_prune.add_argument(
        "--source", help="Only prune sessions from this source"
    )
    sessions_prune.add_argument(
        "--yes", "-y", action="store_true", help="Skip confirmation"
    )
    ...
    sessions_browse.add_argument(
        ...
        type=int,
        default=50,
        help="Max sessions to load (default: 50)",
    )
    ...
    def cmd_sessions(args):
        ...
        if action == "list":
            ...
            if has_titles:
                print(
                    f"{'Title':<32} {'Preview':<40} {'Last Active':<13} {'ID'}"
                )
                print("─" * 110)
            else:
    ...
    completion_parser.set_defaults(
        func=lambda args: cmd_completion(args, parser)
    )
