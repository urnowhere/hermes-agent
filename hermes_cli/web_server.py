...
class OpenDesignConfigUpdate(BaseModel):
    repo_path: str = ""
    start_command: str = ""
    ui_url: str = ""
    health_url: str = ""
...
class OpenDesignBriefPush(BaseModel):
    brief: str
    project_name: str = ""
    skill: str = ""
    design_system: str = ""
...
@app.get("/api/open-design")
async def get_open_design_status():
    from hermes_cli import open_design_integration as od

    return od.status()


@app.get("/api/open-design/config")
async def get_open_design_config():
    from hermes_cli import open_design_integration as od

    return od.load_config()


@app.put("/api/open-design/config")
async def update_open_design_config(body: OpenDesignConfigUpdate):
    from hermes_cli import open_design_integration as od

    try:
        return od.save_config(body.model_dump())
    except Exception as exc:
        raise HTTPException(status_code=400, detail=str(exc))


@app.post("/api/open-design/start")
async def start_open_design():
    from hermes_cli import open_design_integration as od

    try:
        return od.start()
    except Exception as exc:
        _log.exception("POST /api/open-design/start failed")
        raise HTTPException(status_code=400, detail=str(exc))


@app.post("/api/open-design/stop")
async def stop_open_design():
    from hermes_cli import open_design_integration as od

    try:
        return od.stop()
    except Exception as exc:
        _log.exception("POST /api/open-design/stop failed")
        raise HTTPException(status_code=400, detail=str(exc))


@app.post("/api/open-design/restart")
async def restart_open_design():
    from hermes_cli import open_design_integration as od

    try:
        return od.restart()
    except Exception as exc:
        _log.exception("POST /api/open-design/restart failed")
        raise HTTPException(status_code=400, detail=str(exc))


@app.post("/api/open-design/open")
async def open_open_design_ui():
    from hermes_cli import open_design_integration as od
...
@app.get("/api/open-design/logs")
async def get_open_design_logs(lines: int = 200):
    from hermes_cli import open_design_integration as od

    return od.get_logs(lines=lines)


@app.post("/api/open-design/brief")
async def push_open_design_brief(body: OpenDesignBriefPush):
    from hermes_cli import open_design_integration as od

    try:
        return od.push_brief(
            brief=body.brief,
...
@app.get("/api/open-design/artifacts")
async def list_open_design_artifacts(limit: int = 100):
    from hermes_cli import open_design_integration as od

    return {"items": od.list_artifacts(limit=limit)}


@app.get("/api/open-design/artifacts/file")
async def download_open_design_artifact(path: str):
    from hermes_cli import open_design_integration as od

    try:
        target = od.resolve_artifact_path(path)
    except Exception as exc:
        raise HTTPException(status_code=400, detail=str(exc))
