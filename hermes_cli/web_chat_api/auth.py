"""
Hermes Chat UI -- Optional password authentication.
...
# Alias for testing (allows patching)
STATE_DIR = CHAT_STATE_DIR
...
def _signing_key():
    ...
    key_file = CHAT_STATE_DIR / '.signing_key'
    if key_file.exists():
        try:
...
def get_password_hash() -> str | None:
    ...
    # Try loading from settings (if available)
    try:
        from hermes_cli.config import load_config
        settings = load_config()
        webui_settings = settings.get('webui', {})
        return webui_settings.get('password_hash')
    except Exception:
        return None
...
def set_auth_cookie(handler, cookie_value) -> None:
    ...
    if getattr(handler, 'request', None) and getattr(handler.request, 'getpeercert', None) is not None:
