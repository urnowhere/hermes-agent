...
def _install_dependencies(provider_name: str) -> None:
    ...
    for dep in ext_deps:
        ...
        if check_cmd:
            try:
                # Windows cmd.exe does not need shell=True for simple commands,
                # and avoiding it eliminates quoting/escaping differences.
                if sys.platform == "win32":
                    subprocess.run(
                        shlex.split(check_cmd, posix=False),
                        shell=False,
                        capture_output=True,
                        timeout=5,
                    )
            ...
            for pname, _, p in providers:
                if pname == provider_name:
                    if p.is_available():
                        ...
                        print(f"  Status:    not available [ERR]")
