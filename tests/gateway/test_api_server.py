...
class TestRunsEndpointSessionReplay:
    ...
    @pytest.mark.asyncio
    async def test_runs_previous_response_id_reuses_stored_session_id(self, auth_adapter):
        """`/v1/runs` should preserve response-chain session continuity when previous_response_id is provided."""
        stored_history = [
            {"role": "user", "content": "stored message"},
            {"role": "assistant", "content": "stored reply"},
        ]
        auth_adapter._response_store.put("resp_prev", {
            "response": {"id": "resp_prev", "object": "response"},
        ...
        app = _create_app(auth_adapter)
        async with TestClient(TestServer(app)) as cli:
            with patch.object(auth_adapter, "_create_agent", return_value=mock_agent) as create_agent:
                resp = await cli.post(
                    "/v1/runs",
                    headers={"Authorization": "Bearer sk-secret"},
                    json={"input": "new question", "previous_response_id": "resp_prev"},
                )
        ...
        assert create_agent.call_args.kwargs["session_id"] == "stored-session"
        assert create_agent.call_args.kwargs["ephemeral_system_prompt"] == "stored instructions"
        call_kwargs = mock_agent.run_conversation.call_args.kwargs
        assert call_kwargs["conversation_history"] == stored_history
        assert call_kwargs["user_message"] == "new question"
