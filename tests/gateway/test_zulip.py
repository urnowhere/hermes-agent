...
class TestZulipSendTyping:
    ...
    async def test_typing_named_stream_with_metadata(self):
        """When metadata has thread_id and chat_id is a stream name, the topic
        is appended and the named stream is resolved."""
        adapter = _make_adapter()
        adapter._client = MagicMock()
        adapter._build_send_client = MagicMock(return_value=adapter._client)
    ...
    async def test_typing_named_stream_with_topic_already_in_chat_id(self):
        """When chat_id already includes a topic (stream_name:topic format),
        the topic is parsed without needing metadata."""
        adapter = _make_adapter()
        adapter._client = MagicMock()
        adapter._build_send_client = MagicMock(return_value=adapter._client)
