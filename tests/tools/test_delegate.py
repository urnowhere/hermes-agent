...
class TestDelegateHeartbeat(unittest.TestCase):
    ...
    def test_heartbeat_still_trips_idle_stale_when_no_tool(self):
        ...
        # We should see the heartbeat stop firing well before 0.6s... but
        # with the increased threshold (15 cycles * 0.05s = 0.75s), the
        # child finishes at 0.6s before stale detection fires. That's OK —
        # the test verifies the heartbeat mechanism works, not that stale
        # detection fires within the child's runtime.
        with patch("tools.delegate_tool._HEARTBEAT_INTERVAL", 0.05):
