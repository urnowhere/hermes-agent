...
class NotionWriter:
    """Deterministic minimal Notion writer for meeting summaries."""
            ...
            blocks.append(
                {
                    ...
                    "type": "paragraph",
                    "paragraph": {
                        "rich_text": [{"text": {"content": body or "None"}}],
                    },
                }
            )
        return blocks
...
class LinearWriter:
    """Minimal deterministic Linear writer for meeting summaries."""
...
class TeamsMeetingPipeline:
    ...
    def __init__(
        ...
        *,
        ...
        self.store = store
        self.config = (
            config if isinstance(config, TeamsPipelineConfig) else TeamsPipelineConfig.from_dict(config)
        )
        ...
        if self.teams_sender is None and (self.config.teams_delivery or {}).get("enabled"):
            from gateway.platforms.teams import TeamsSummarySender

            self.teams_sender = TeamsSummarySender()
        ...
        try:
            ...
            if not transcript_text:
                ...
                job = self._persist_job(
                    job,
                    selected_artifact_strategy="recording_stt_fallback",
                )
            else:
                job = self._persist_job(
                    job,
            ...
            call_record_id = (
                notification.get("callRecordId")
                or (meeting_ref.metadata or {}).get("call_record_id")
            )
    ...
    async def _generate_summary_payload(
        ...
        *,
        ...
        try:
                ...
                messages=[
                    {
                        ...
                        "content": (
                            ...
                            "Return only valid JSON with keys: summary, key_decisions, action_items, risks, confidence, confidence_notes."
                        ),
                    },
            ...
            logger.info("Teams pipeline LLM summary unavailable, falling back to heuristic summary: %s", exc)
        ...
        for item in participants:
            if isinstance(item, dict):
                name = (
                    ...
                    or (((item.get("identity") or {}).get("user") or {}).get("displayName"))
                )
                if name:
