...
def _chromium_installed() -> bool:
    ...
    2. Playwright's browser cache — directories containing
       ``chromium-*`` or ``chromium_headless_shell-*``.

    agent-browser (0.26+) downloads Playwright's chromium / headless-shell
    builds into ``PLAYWRIGHT_BROWSERS_PATH`` and won't start without a browser
    binary.  Without a browser binary the CLI
