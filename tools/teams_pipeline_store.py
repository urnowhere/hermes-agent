"""Durable local state for the Microsoft Teams meeting pipeline.

Stores three kinds of state:
- Graph subscription metadata
- Notification receipts for idempotency / dedupe
- Event timestamps for pipeline bookkeeping
"""
...
def resolve_teams_pipeline_store_path(
    path: str | Path | None = None,
    *,
    platform_extra: Optional[Dict[str, Any]] = None,
) -> Path:
    """Resolve the durable Teams pipeline store path from explicit input or config."""
    if path is not None:
    ...
    if platform_extra:
        configured = str(platform_extra.get("store_path") or "").strip()
        if configured:
