...
class AIAgent:
    ...
    def run_conversation(
        ...
        # ── System prompt vs context length validation ──
        # The system prompt (persona + tools + memory + skills) is typically
        # 8-15K tokens.  If the model is loaded with a context window smaller
        # than the system prompt, the server rejects the request with
        # "n_keep >= n_ctx" or "Compute error".  Catch this early and warn.
        if active_system_prompt and hasattr(self, "context_compressor"):
            _ctx_len = getattr(self.context_compressor, "context_length", 0)
            if _ctx_len:
                _sys_est = estimate_tokens_rough(active_system_prompt)
                if _sys_est >= _ctx_len:
                    _msg = (
                        f"System prompt (~{_sys_est:,} tokens) exceeds model context "
                        f"({_ctx_len:,} tokens). The server will reject this request. "
                        f"Load the model with context_length >= {_sys_est + 2048:,} or "
                        f"reduce the number of tools/skills loaded."
                    )
                    logger.error(_msg)
                    if not self.quiet_mode:
                        self._safe_print(f"❌ {_msg}")
