                ...
                if delta and delta.tool_calls:
                    for tc_delta in delta.tool_calls:
                        ...
                        if (
                            ...
                            # LM-Studio fix: detect fragmented tool calls where
                            # each argument chunk has a unique id but empty name
                            # and short arguments. These are fragments of a
                            # single tool call, not separate calls.
                            func = tc_delta.function
                            is_fragment = (
                                func
                                and not func.name
                                and func.arguments
                                and len(func.arguments) < 100
                            )
                            if is_fragment:
                                # This is a fragment of the current tool call.
                                # Keep using the active slot for this index
                                # (already updated when the name chunk arrived).
                                pass
                            else:
                                # Genuinely new tool call (Ollama pattern or
                                # LM-Studio name chunk).
