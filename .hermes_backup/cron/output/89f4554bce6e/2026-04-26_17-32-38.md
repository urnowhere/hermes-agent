...
- ✅ skills — 完整同步
- ✅ cron — jobs.json + output/ 目录
- ✅ databases — state.db + dongchedi_l90.db + whatsapp 两个 DB

**Git 状态：**

- Commit: `17d632f2 backup hermes 2026-04-26`
- 同步状态: `## main...origin/main`（无 ahead，已同步）

**注意：** `state.db` 因 `sqlite3` CLI 不可用，改用 `PRAGMA wal_checkpoint(PASSIVE)` + `shutil.copy2` 替代 `.backup` 命令，效果相同。
