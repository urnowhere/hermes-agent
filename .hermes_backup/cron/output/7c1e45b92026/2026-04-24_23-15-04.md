...
### 新增车源（单独标注）
- new_count > 0 时，单独列出所有新增车源的完整信息

### 下架车源（单独标注）
- removed_count > 0 时，单独列出所有下架车源的完整信息
...
4. **重试逻辑**：如果 send_message 失败，等 5 秒重试，最多 3 次
...
The `dongchedi_watch` tool is not available in my current toolset. No browser, mcporter, or car-scraper tools are accessible. Cannot execute the monitoring task.
