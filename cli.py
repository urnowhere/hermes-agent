...
_IMAGE_EXT_RE = "|".join(
    re.escape(ext.lstrip(".")) for ext in sorted(_IMAGE_EXTENSIONS)
)
_QUOTED_INLINE_IMAGE_PATH_RE = re.compile(
    rf"(?P<quote>['\"])(?P<path>.*?\.({_IMAGE_EXT_RE}))(?P=quote)",
    re.IGNORECASE,
)
_UNQUOTED_INLINE_IMAGE_PATH_RE = re.compile(
    rf"(?<!\S)(?P<path>(?:file://|~|/|\./|\../|[A-Za-z]:[\\/])"
    rf"(?:\\ |[^\s'\"<>])+?\.({_IMAGE_EXT_RE}))(?![A-Za-z0-9_])",
    re.IGNORECASE,
)
...
def _collect_inline_image_paths(text: str) -> list[Path]:
    """Return existing local image paths referenced anywhere in free-form text.

    ``_detect_file_drop`` intentionally handles leading paths only because it
    also splits off trailing prompt text.  This helper is for prompts like
    ``Translate this page: /tmp/page.jpg`` where the image is a referenced
    input, not the first token.  The caller keeps the original text unchanged
    and attaches the pixels separately.
    """
    ...
    for match in _QUOTED_INLINE_IMAGE_PATH_RE.finditer(text):
        _add(match.group("path"))

    for match in _UNQUOTED_INLINE_IMAGE_PATH_RE.finditer(text):
        _add(match.group("path"))
...
def _collect_query_images(
    ...
    if isinstance(message, str):
        ...
        if dropped and dropped.get("is_image"):
            ...
            images.extend(_collect_inline_image_paths(message))
...
def save_provider_model_thinking_mode(
    provider_key: str,
    model_name: str,
    enable_thinking: bool,
) -> bool:
    """Persist ``chat_template_kwargs.enable_thinking`` for one provider/model."""
    ...
    project_config_path = Path(__file__).parent / "cli-config.yaml"
    config_path = (
        user_config_path if user_config_path.exists() else project_config_path
    )
    ...
    try:
        ...
        if not isinstance(chat_template_kwargs, dict):
            ...
            model_entry["chat_template_kwargs"] = chat_template_kwargs
        ...
        logger.error("Failed to save provider/model thinking mode: %s", e)
        return False


def clear_provider_model_thinking_mode(provider_key: str, model_name: str) -> bool:
    """Remove a persisted provider/model thinking override."""
    provider_key = (provider_key or "").strip()
    model_name = (model_name or "").strip()
    if not provider_key or not model_name:
        return False
    ...
    try:
        ...
        if not chat_template_kwargs:
            model_entry.pop("chat_template_kwargs", None)
        ...
        logger.error("Failed to clear provider/model thinking mode: %s", e)
        return False
...
class HermesCLI:
    ...
    def __init__(
        ...
        self._provider_status_file: Optional[str] = None
        self._provider_status_text: str = ""
        self._provider_status_poll_stop: Optional[threading.Event] = None
        self._provider_status_poll_thread: Optional[threading.Thread] = None
    ...
    def _render_spinner_text(self) -> str:
        ...
        provider_status = getattr(self, "_provider_status_text", "")
        if not txt and not provider_status:
            return ""
        parts = []
        if txt:
            ...
            if t0 > 0:
                ...
                txt = f"{txt} ({elapsed_str})"
            parts.append(txt)
        if provider_status:
            parts.append(provider_status)
        return f"  {'  ·  '.join(parts)}"

    def _read_provider_status_snapshot(self) -> Optional[Dict[str, Any]]:
        path = getattr(self, "_provider_status_file", None)
    ...
    @staticmethod
    def _format_provider_status_brief(snapshot: Optional[Dict[str, Any]]) -> str:
        if not snapshot:
            return ""
        state = str(snapshot.get("state") or "stopped").strip().lower()
        model = str(snapshot.get("selected_model") or "").strip()
        last_error = str(snapshot.get("last_error") or "").strip()
        if state == "ready":
            return f"stack: ready {model}" if model else "stack: ready"
        if state == "starting":
            return f"stack: starting {model}" if model else "stack: starting"
        if state == "failed":
            detail = model or "model"
            if last_error:
                detail = f"{detail} ({last_error[:80]})"
            return f"stack: failed {detail}"
        return "stack: stopped"

    def _refresh_provider_status_text(self) -> Optional[Dict[str, Any]]:
        snapshot = self._read_provider_status_snapshot()
        if snapshot:
            self._provider_status_text = self._format_provider_status_brief(snapshot)
            return snapshot
        self._provider_status_text = ""
        return None

    def _start_provider_status_poller(self) -> None:
        self._stop_provider_status_poller(clear_text=True)
        if not getattr(self, "_provider_status_file", None):
            return
        self._refresh_provider_status_text()
        stop_event = threading.Event()
        self._provider_status_poll_stop = stop_event
        ...
        thread = threading.Thread(
            target=_poll,
            daemon=True,
            name="provider-status-poller",
        )
        self._provider_status_poll_thread = thread
        thread.start()

    def _stop_provider_status_poller(self, *, clear_text: bool = False) -> None:
        stop_event = getattr(self, "_provider_status_poll_stop", None)
        thread = getattr(self, "_provider_status_poll_thread", None)
        if stop_event is not None:
            stop_event.set()
        if thread is not None and thread.is_alive():
            thread.join(timeout=1.0)
        self._provider_status_poll_stop = None
        self._provider_status_poll_thread = None
        if clear_text:
            self._provider_status_text = ""

    def _resolve_thinking_provider_key(self) -> str:
        source = str(getattr(self, "_provider_source", "") or "").strip()
        ...
        if requested and requested.lower() != "auto":
            return requested
        return str(getattr(self, "provider", "") or "").strip()

    def _load_persisted_thinking_mode(self) -> Optional[bool]:
        provider_key = self._resolve_thinking_provider_key()
        ...
        if provider_entry is None:
            for entry in cfg.get("custom_providers") or []:
                if not isinstance(entry, dict):
                    continue
                if str(entry.get("name") or "").strip() == provider_key:
    ...
    def _apply_persisted_thinking_mode(self, agent_obj: Any = None) -> Optional[bool]:
        enable = self._load_persisted_thinking_mode()
        target = agent_obj if agent_obj is not None else getattr(self, "agent", None)
        if target is None:
            return enable
        if enable is None:
            target._chat_template_kwargs = None
        else:
            target._chat_template_kwargs = {"enable_thinking": enable}
        return enable

    def _current_thinking_mode_label(self) -> str:
        agent = getattr(self, "agent", None)
        ctk = getattr(agent, "_chat_template_kwargs", None) if agent is not None else None
        if isinstance(ctk, dict) and ctk.get("enable_thinking") is True:
            return "on"
        if isinstance(ctk, dict) and ctk.get("enable_thinking") is False:
            return "off"

        persisted = self._load_persisted_thinking_mode()
    ...
    def _format_submitted_user_message_preview(self, user_input: str) -> str:
        ...
        preview_lines = [f"[bold {_accent_hex()}]●[/] [bold]{_escape(head[0])}[/]"]
    ...
    def _ensure_runtime_credentials(self) -> bool:
        ...
        self._provider_headers = runtime.get("headers")
        self._provider_status_file = runtime.get("status_file")
        self._provider_status_text = ""
    ...
    def _resolve_turn_agent_config(self, user_message: str) -> dict:
        ...
        route = {
            ...
            "signature": (
                ...
                runtime["command"],
                tuple(runtime["args"]),
                tuple(sorted((runtime.get("provider_headers") or {}).items())),
            ),
        }
        ...
        *,
        ...
        try:
            ...
            self.agent = AIAgent(
                ...
                provider_headers=runtime.get("provider_headers")
                or runtime.get("headers"),
            ...
            self._apply_persisted_thinking_mode(self.agent)
    ...
    def _show_status(self):
        ...
        stack_snapshot = self._read_provider_status_snapshot()
        stack_info = ""
        if stack_snapshot:
            stack_info = (
                f" [dim {separator_color}]·[/] [dim]"
                f"{self._format_provider_status_brief(stack_snapshot)}[/]"
            )
        thinking_info = (
            f" [dim {separator_color}]·[/] [dim]"
            f"thinking: {self._current_thinking_mode_label()}[/]"
        )

        self._console_print(
            f"  {api_indicator} [{accent_color}]{model_short}[/] "
            f"[dim {separator_color}]·[/] [bold {label_color}]{tool_count} tools[/]"
            f"{toolsets_info}{provider_info}{stack_info}{thinking_info}"
        )
    ...
    def _handle_branch_command(self, cmd_original: str) -> None:
        ...
        if self.agent:
            ...
            if hasattr(self.agent, "session_log_file") and hasattr(
                self.agent, "logs_dir"
            ):
    ...
    def process_command(self, command: str) -> bool:
        ...
        if canonical in ("quit", "exit", "q"):
            ...
            self._handle_thinking_command(cmd_original)
        ...
        def run_background():
            ...
            try:
                bg_agent = AIAgent(
                    ...
                    provider_headers=turn_route["runtime"].get("provider_headers"),
                    max_iterations=self.max_turns,
                    enabled_toolsets=self.enabled_toolsets,
                    quiet_mode=True,
            ...
            if _already_open:
                ...
                if _launched:
                    ...
                    if chrome_cmd:
                        ...
                        print(
                            "     No Chrome/Chromium executable found in this environment"
                        )
            else:
    ...
    def _maybe_continue_goal_after_turn(self) -> None:
        ...
        try:
            if (
                getattr(self, "_pending_input", None) is not None
    ...
    def _handle_thinking_command(self, cmd: str):
        """Handle /thinking [on|off|default|status] — toggle chat_template_kwargs.enable_thinking.

        Works with buun-server and any backend that accepts chat_template_kwargs
        in extra_body. Persists per provider/model and is applied to new agents
        automatically; live agents update immediately for the next message.
        """
        parts = cmd.strip().split(maxsplit=1)
        arg = parts[1].strip().lower() if len(parts) > 1 else "status"

        if not self._ensure_runtime_credentials():
            _cprint("  (._.) Could not resolve the active provider/model.")
            return

        if arg == "status":
            state = self._current_thinking_mode_label()
            if state == "default":
                state = "default (server decides)"
            else:
                state = state.upper()
            _cprint(f"  {_ACCENT}Thinking mode: {state}{_RST}")
            _cprint(f"  {_DIM}Usage: /thinking [on|off|default|status]{_RST}")
            return
        ...
        if arg in {"on", "true", "1"}:
            ...
            model_name = str(getattr(self, "model", "") or "").strip()
            if not provider_key or not model_name:
                _cprint("  (._.) Could not determine the active provider/model.")
                return
            if getattr(self, "agent", None) is not None:
                self.agent._chat_template_kwargs = None
            if clear_provider_model_thinking_mode(provider_key, model_name):
                _cprint(
                    f"  {_ACCENT}✓ Thinking mode reset to server default for {provider_key} / {model_name} (saved){_RST}"
                )
            else:
                _cprint(
                    f"  {_ACCENT}✓ Thinking mode reset to server default for {provider_key} / {model_name} (session only){_RST}"
                )
            _cprint(f"  {_DIM}This takes effect on the next message.{_RST}")
            return
        else:
            _cprint(f"  {_DIM}(._.) Unknown argument: {arg!r}{_RST}")
            _cprint(f"  {_DIM}Usage: /thinking [on|off|default|status]{_RST}")
            return
        ...
        if getattr(self, "agent", None) is not None:
            self.agent._chat_template_kwargs = {"enable_thinking": enable}

        if save_provider_model_thinking_mode(provider_key, model_name, enable):
            _cprint(
                f"  {_ACCENT}✓ Thinking mode set to {label} for {provider_key} / {model_name} (saved){_RST}"
            )
        else:
            _cprint(
                f"  {_ACCENT}✓ Thinking mode set to {label} for {provider_key} / {model_name} (session only){_RST}"
            )
        _cprint(f"  {_DIM}This takes effect on the next message.{_RST}")
    ...
    def chat(self, message, images: list = None) -> Optional[str]:
        ...
        try:
            ...
            self._start_provider_status_poller()
    ...
    def run(self):
            ...
            while not self._should_exit:
                try:
                    ...
                    if (
                        ...
                        _inline_images = _collect_inline_image_paths(user_input)
                        _added_inline_images = []
                        _existing_image_keys = {str(p) for p in submit_images}
                        for _img in _inline_images:
                            _key = str(_img)
                            if _key in _existing_image_keys:
                                continue
                            submit_images.append(_img)
                            _existing_image_keys.add(_key)
                            _added_inline_images.append(_img)
                        if _added_inline_images:
                            _names = ", ".join(
                                p.name for p in _added_inline_images[:3]
                            )
                            if len(_added_inline_images) > 3:
                                _names += (
                                    f", +{len(_added_inline_images) - 3} more"
                                )
                            _cprint(f"  📎 Auto-attached image reference: {_names}")
        ...
        try:
            ...
            if (
                ...
                and getattr(_stdin_err, "errno", None) == errno.EIO
            ):
                pass  # suppress broken-stdout I/O errors on interrupt (#13710)
            elif "is not registered" in str(_stdin_err) or "Bad file descriptor" in str(
                _stdin_err
            ):
                print(
