...
display:
  ...
  language: en            # UI language for static messages (approval prompts, some gateway replies). en | zh | ja | de | es | fr | pt | pt_BR | tr | uk
```
...
Supported values: `en` (default), `zh` (Simplified Chinese), `ja` (Japanese), `de` (German), `es` (Spanish), `fr` (French), `pt` (Portuguese), `pt_BR` (Brazilian Portuguese), `tr` (Turkish), `uk` (Ukrainian). Unknown values fall back to English.
